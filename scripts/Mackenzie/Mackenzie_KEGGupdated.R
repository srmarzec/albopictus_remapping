...
genes_with_pvalues <- "../misc/72h_DvND_LFCshrink_padj.txt" # with at least columns "gene", "padj"

keggGeneID_output <- "../misc/72h_DvND_keggID.txt"
...
geneList <- all_genes_list$X72h_padj #CHANGE THIS LINE
geneLFClist <- all_genes_list$X72h_Log2FoldChange #CHANGE THIS LINE
...
#genes.by.pathway_72h <- genes.by.pathway[-c(99, 120)]

pathway_pval <- data.frame()

#for below portion of script still get warning that there are 50 or more warnings (says that condition has length over 1 and only first element will be used), but runs 
...
write.csv(pathway_pval, 
          file="../misc/larva_72h_keggPathwayEnrichment.csv", row.names = F)
...
genes_with_pvalues <- "../misc/135h_DIvNDI_LFCshrink_padj.txt" # with at least columns "gene", "padj"

keggGeneID_output <- "../misc/135h_DvND_keggID.txt"
...
geneList <- all_genes_list$X135h_padj #CHANGE THIS LINE
geneLFClist <- all_genes_list$X135h_Log2FoldChange #CHANGE THIS LINE
names(geneList) <- sub("aalb:","", all_genes_list$kegg_id) # get rid of the beginning "aalb:" since the gene list we brought from kegg doesn't have this
names(geneLFClist) <- sub("aalb:","", all_genes_list$kegg_id)
head(geneList)

#genes.by.pathway_135h <- genes.by.pathway[-c(99, 120)]
